...
<form id="delete-image-{{ $img->id }}" action="{{ route('admin.products.images.destroy', $img) }}" method="POST" style="display:none;">
    @csrf @method('DELETE')
</form>
@endforeach
...
<div class="form-grid">
    ...
    <div class="form-group">
        ...
        <input type="number" name="discount_percent" class="form-input" value="{{ old('discount_percent', $product->discount_percent ?? 0) }}" min="0" max="100" placeholder="0">
        <p class="form-hint">Berlaku untuk harga dasar. Variasi punya harga sendiri.</p>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="toggle-wrap" style="margin-top:10px;">
            <input type="checkbox" name="is_featured" class="toggle" id="isFeatured" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }}>
    ...
    <div class="form-group full" style="background:#fafaf8;border:.5px solid rgba(11,42,74,.07);border-radius:12px;padding:20px 24px;">
        @include('admin.products._variant_builder', ['product' => $product])
    </div>
    {{-- ─────────────────────────────────────────────────────────── --}}
