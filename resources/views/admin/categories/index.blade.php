...
.cat-table td {
    ...
    font-size: 13px;
    color: #0b2a4a;
    border-bottom: 0.5px solid rgba(11,42,74,0.04);
    vertical-align: middle;
}
.cat-table tr:last-child td { border-bottom: none; }

.row-parent td { background: rgba(11,42,74,0.015); }
.row-parent td:first-child { position: relative; }
...
.form-divider {
    border: none;
    border-top: 0.5px solid rgba(11,42,74,0.08);
    margin: 22px 0;
}
...
.section-sep td {
    ...
    text-transform: uppercase;
    color: rgba(11,42,74,0.25);
    background: rgba(11,42,74,0.02);
    border-top: 0.5px solid rgba(11,42,74,0.05);
    border-bottom: none;
}
...
<div class="page-layout">
    ...
    <div class="card">
        ...
        <table class="cat-table">
            ...
            <tbody>
                {{--
                    Tampilkan parent categories dulu, lalu children-nya masing-masing.
                    Controller harus kirim $categories sudah di-eager load children.
                    Contoh di controller:
                        $categories = Category::with(['children' => fn($q) => $q->withCount('products')])
                                        ...
                                        ->orderBy('sort')
                                        ->get();
                    Kalau controller kamu masih pakai ->get() flat, ganti dulu supaya view ini bisa jalan.
                --}}
                ...
                @forelse($categories as $cat)
                    ...
                    <tr class="row-parent">
                        ...
                        <td>
                            <div class="action-group">
                                <button class="btn-sm btn-sm-accent"
                                    onclick="openAddSub({{ $cat->id }}, '{{ addslashes($cat->name) }}')">
                                    + Sub
                                </button>
                                <button class="btn-sm"
                                    onclick="openEdit({{ $cat->id }}, '{{ addslashes($cat->name) }}', '{{ addslashes($cat->icon) }}', {{ $cat->sort }}, null, null)">
                                    Edit
                                </button>
                                <button class="btn-sm btn-sm-danger"
                                    onclick="confirmDelete('del-cat-{{ $cat->id }}', {{ $cat->children ? $cat->children->count() : 0 }})">
                                    Hapus
                                </button>
                            </div>
                        </td>
                    ...
                    @if($cat->children && $cat->children->count() > 0)
                        @foreach($cat->children as $child)
                            <form id="del-cat-{{ $child->id }}" action="{{ route('admin.categories.destroy', $child) }}" method="POST" style="display:none;">
                                @csrf @method('DELETE')
                            </form>
                            <form id="toggle-cat-{{ $child->id }}" action="{{ route('admin.categories.toggle', $child) }}" method="POST" style="display:none;">
                                @csrf
                            </form>

                            <tr class="row-child">
                                <td style="padding-left:22px;">
                                    <span class="cat-icon" style="font-size:15px;">{{ $child->icon ?: '—' }}</span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="action-group">
                                        <button class="btn-sm"
                                            onclick="openEdit({{ $child->id }}, '{{ addslashes($child->name) }}', '{{ addslashes($child->icon) }}', {{ $child->sort }}, {{ $child->parent_id }}, '{{ addslashes($cat->name) }}')">
                                            Edit
                                        </button>
                                        <button class="btn-sm btn-sm-danger"
                                            onclick="confirmDelete('del-cat-{{ $child->id }}', 0)">
                                            Hapus
                                        </button>
                                    </div>
                                </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="empty-state">
                            Belum ada kategori. Tambah kategori utama di panel kanan.
                        </td>
                    </tr>
                @endforelse
            </tbody>
        ...
        <div class="tab-panel active" id="tab-parent">
            <form action="{{ route('admin.categories.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-input" placeholder="Contoh: Fashion" required
                           value="{{ old('name') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="sort" class="form-input" placeholder="0" value="{{ old('sort', 0) }}" min="0">
        ...
        <div class="tab-panel" id="tab-sub">
            <form action="{{ route('admin.categories.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <p class="form-hint">Sub-kategori akan muncul di bawah kategori ini.</p>
                </div>
                <div class="form-group">
        ...
        <form id="editForm" method="POST">
            ...
            <div style="display:flex;gap:10px;margin-top:20px;">
                <button type="button" onclick="closeEdit()"
                    style="flex:1;padding:10px;border:0.5px solid rgba(11,42,74,0.15);border-radius:8px;background:none;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:12px;color:rgba(11,42,74,0.6);">
                    Batal
                </button>
...
<div class="modal-overlay" id="addSubModal">
    <div class="modal-box">
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST">
            ...
            <div style="display:flex;gap:10px;margin-top:20px;">
                <button type="button" onclick="closeAddSub()"
                    style="flex:1;padding:10px;border:0.5px solid rgba(11,42,74,0.15);border-radius:8px;background:none;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:12px;color:rgba(11,42,74,0.6);">
                    Batal
                </button>
                <button type="submit" class="btn-submit" style="flex:2;">Tambah Sub-Kategori</button>
            </div>
        </form>
    </div>
</div>
...
function openEdit(id, name, icon, sort, parentId, parentName) {
    ...
    const modalSub    = document.getElementById('editModalSubtitle');

    if (parentId) {
        parentGroup.style.display = 'block';
        parentSel.value = parentId;
        modalTitle.textContent = 'Edit Sub-Kategori';
        modalSub.textContent   = 'Di bawah: ' + (parentName || '');
    } else {
        parentGroup.style.display = 'none';
        parentSel.value = '';
        modalTitle.textContent = 'Edit Kategori Utama';
...
function confirmDelete(formId, childrenCount) {
    let msg = 'Hapus kategori ini?';
    if (childrenCount > 0) {
        msg = 'Kategori ini punya ' + childrenCount + ' sub-kategori. Hapus semuanya?';
    }
    if (confirm(msg)) {
