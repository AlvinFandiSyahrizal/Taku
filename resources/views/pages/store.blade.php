...
@php
    ...
    $baseUrl = route('store.show', $store->slug);
@endphp
...
.owner-notice{background:#eae4d8;border-bottom:1px solid var(--sand);padding:10px 32px;display:flex;align-items:center;justify-content:space-between;gap:12px;font-family:'DM Sans',sans-serif;}
...
.btn-notice{display:inline-flex;align-items:center;gap:5px;padding:6px 14px;border-radius:6px;font-size:10px;letter-spacing:.1em;text-transform:uppercase;font-weight:500;text-decoration:none;font-family:'DM Sans',sans-serif;transition:all .2s;}
...
.store-avatar-init{width:80px;height:80px;border-radius:50%;background:var(--sand-lt);border:2px solid var(--sand);display:flex;align-items:center;justify-content:center;font-family:'Cormorant Garamond',serif;font-size:36px;color:var(--olive-dk);flex-shrink:0;}
...
.badge-active{background:#eaf0e4;color:var(--olive-dk);border:.5px solid #c0ceb0;font-size:9px;letter-spacing:.12em;text-transform:uppercase;padding:5px 14px;border-radius:100px;font-family:'DM Sans',sans-serif;}
.owner-badge{background:var(--sand-lt);color:var(--olive-dk);border:.5px solid var(--sand);font-size:9px;letter-spacing:.12em;text-transform:uppercase;padding:5px 14px;border-radius:100px;font-family:'DM Sans',sans-serif;}
...
.btn-wa:hover{background:var(--olive) !important;color:var(--olive-lt) !important;transform:translateY(-1px);}
.btn-manage{display:inline-flex !important;align-items:center;gap:7px;padding:11px 20px;background:transparent !important;color:var(--olive-dk) !important;border:1px solid var(--sand) !important;border-radius:8px;font-size:11px;letter-spacing:.08em;text-transform:uppercase;text-decoration:none !important;font-family:'DM Sans',sans-serif;font-weight:500;transition:all .2s;}
.btn-manage:hover{background:var(--sand-lt) !important;border-color:var(--olive) !important;}
...
.cat-link.active .cat-chevron{color:var(--olive-lt);}
...
.sort-btn{padding:6px 14px;border-radius:100px;font-size:10px;letter-spacing:.08em;text-transform:uppercase;text-decoration:none;border:1px solid var(--sand);color:var(--stone);background:var(--cream);font-family:'DM Sans',sans-serif;transition:all .2s;white-space:nowrap;cursor:pointer;}
.sort-btn:hover{color:var(--olive-dk);border-color:var(--olive);background:var(--sand-lt);}
.sort-btn.active{background:var(--olive-dk);color:var(--olive-lt);border-color:var(--olive-dk);}
...
.prod-card-disc{font-size:9px;background:rgba(74,89,64,.1);color:var(--olive-dk);padding:2px 5px;border-radius:100px;letter-spacing:.03em;}
...
@media(max-width:640px){
    ...
    .sort-bar .sort-btn{display:none;}
...
<div class="owner-notice">
    <p class="owner-notice-text">
        ...
        Kamu sedang melihat toko kamu sebagai <strong>pemilik</strong>.
    </p>
...
<div class="store-hero">
    ...
    <div class="store-info">
        ...
        <div class="store-meta">
            ...
            @endif
            @if($store->city)
                <span class="store-meta-item">📍 {{ $store->city }}</span>
            @endif
        </div>
    </div>
...
@if($bannersTop->count() > 0)
    @include('partials.store-banner-slider', ['banners' => $bannersTop, 'sliderId' => 'bannerTop'])
@endif
...
<div class="store-body">
    ...
    <div style="margin-bottom:48px;border-radius:12px;overflow:hidden;">
        @include('partials.store-banner-slider', ['banners' => $bannersAfterSections, 'sliderId' => 'bannerMid'])
    </div>
    @endif
            ...
            <ul class="cat-list">
                <li>
                    <a href="{{ $baseUrl }}" class="cat-link {{ !$categorySlug && !$subSlug ? 'active' : '' }}">
                        ...
                        <span style="font-size:10px;color:inherit;opacity:.7;">{{ $totalProducts }}</span>
                    </a>
                </li>
                ...
                @php
                    ...
                    $catProdCount = $cat->products()->where('store_id', $store->id)->where('is_active', true)->count();
                @endphp
                <li>
                    <a href="{{ $baseUrl }}?category={{ $cat->slug }}"
                       ...
                       @if($hasChildren) onclick="toggleSub(event, 'sub-{{ $cat->id }}')" @endif>
                        ...
                        @else
                            <span style="font-size:10px;color:inherit;opacity:.6;">{{ $catProdCount }}</span>
                        @endif
                    </a>
                    @if($hasChildren)
                    <ul class="sub-cat-list {{ $isExpanded ? 'open' : '' }}" id="sub-{{ $cat->id }}">
                        @foreach($cat->children as $child)
                        @php $childCount = $child->products()->where('store_id', $store->id)->where('is_active', true)->count(); @endphp
                        <li>
        ...
        <div class="catalog-main">
            ...
            <div class="catalog-topbar">
                <div class="catalog-info">
                    ...
                    @if($activeSubCategory)
                        dalam <strong>{{ $activeSubCategory->name }}</strong>
                    @elseif($activeCategory)
                        dalam <strong>{{ $activeCategory->name }}</strong>
                    @endif
                </div>
                <div class="sort-bar">
            ...
            <div class="pagination-wrap">
                ...
                @else
                    <a href="{{ $products->previousPageUrl() }}" class="pg-btn" onclick="scrollToCatalog(event, this.href)">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
                    </a>
                @endif
                ...
                @if($products->hasMorePages())
                    <a href="{{ $products->nextPageUrl() }}" class="pg-btn" onclick="scrollToCatalog(event, this.href)">
            ...
            <div class="empty-wrap">
                ...
                <p style="font-size:13px;font-family:'DM Sans',sans-serif;">
                    {{ $categorySlug || $subSlug ? 'Tidak ada produk di kategori ini.' : 'Toko ini belum menambahkan produk.' }}
                </p>
            </div>
            @endif
        </div>
    ...
    <div style="margin-top:48px;border-radius:12px;overflow:hidden;">
        @include('partials.store-banner-slider', ['banners' => $bannersBottom, 'sliderId' => 'bannerBot'])
    </div>
    @endif
...
function bindPagination() {
    document.querySelectorAll('.pg-btn:not(.disabled):not(.active)').forEach(btn => {
        btn.onclick = (e) => scrollToCatalog(e, btn.href);
    });
}
